...
function PadC( cIn : string; nLength : integer; cFillChar : char ) : string;
...
function GetSQLDateS( InD : TDateTime ) : string;

implementation

uses StrUtils, SysUtils, NAV, Winapi.Windows;
...
begin
	cIn := Trim( cIn );
	cSeged := cIn;
	for I := 1 to ( nLength - Length( cIn )) div 2 do begin
		cSeged := cFillChar + cSeged + cFillChar;
	end;
	Result := Copy( cSeged,1,nLength );
end;
